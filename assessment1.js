...
async function startTest() {
    showLoading();
    loadingMessage.innerText = "Prepare-se para avaliar a maturidade relacional"
...
const sectionInterpretations = {
    ...
    "2. FREQUÊNCIA": {
        ...
        good:
            "<strong>Boa Frequência</strong>:\nA relação é ativa, mas pode haver momentos de menor contato ou períodos mais espaçados de interação. A comunicação ainda ocorre de maneira frequente, mas sem um padrão fixo. Em alguns momentos, a conexão pode parecer forte, enquanto em outros há um distanciamento natural. ➡ <strong>Ações recomendadas</strong>: Busque criar um padrão mais previsível de interação. Pequenos ajustes, como programar encontros regulares ou estabelecer um fluxo de comunicação mais contínuo, podem tornar essa relação ainda mais sólida. Se a relação for estratégica, um esforço intencional para manter a frequência pode evitar que ela perca relevância ao longo do tempo.",
        average:
...
async function showResults() {
    ...
    const messages = [
        ...
        "Calculando seu Índice de Maturidade Relacional",
    ...
    finalMessageTitle.innerText = "IMR - Índice de Maturidade Relacional";
...
totalResultTitle.innerText = "Seu Índice de Maturidade Relacional foi de:";
...
//  saveAssessment("Índice de Maturidade Relacional", totalScore, getResultInterpretation(totalScore)); - OLD CALL
await saveAssessment("Índice de Maturidade Relacional", totalScore, interpretation, testAnswers);
    ...
    As conexões profissionais não são estáticas – elas evoluem conforme o tempo, as interações e o valor que cada parte deposita na relação. O Índice de Maturidade Relacional (IMR) oferece um diagnóstico do momento, mas a construção e a manutenção de relações de qualidade são processos contínuos, que dependem de intenção, consistência e autenticidade. Como próximos passos:
    <ul>
    ...
    <br><br>
    Para saber mais sobre o IMR (Índice de Maturidade Relacional), você pode acessar os materiais de aprofundamento gratuitos no site <a href="https://www.innernetworking.com.br" target="_blank">www.innernetworking.com.br</a>. Além disso, caso seja do seu interesse, você pode obter uma versão detalhada do seu IMR e mesmo desenvolver o seu Desafio Perfeito para dar um salto evolutivo em seu nível de networking.
    <br><br>
...
async function shareOnWhatsApp() {
    const message = `Olá! Acabei de fazer um teste de Índice de Maturidade Relacional incrível e olha só o resultado:`;
